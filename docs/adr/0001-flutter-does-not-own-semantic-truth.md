...
responsiveness. Doing so creates a second implementation of the language
that drifts from the compiler, and moves semantic decisions into widgets.
...
semantic fact. Studio state is split into semantic projection / editor
state / rendering state; only the first comes from Rust and Studio never
mutates it.
