...
| `f(x)`                                                                                                                                                                                                                                                                                                                                  | call of a relationship                    | `app (declRef f) x`; arguments are Sem values (an input, a relationship's value), never bare numbers                                                            |
...
| `match` on Bool, Option (nested), dimensionless numbers, a Sem value through its representation | yes                    | yes, with conservative exhaustiveness (`formula.match.non_exhaustive`) and unreachable-arm warnings                                    | yes                                                 |
...
| `==` on truth values, counts, optional values, collections, grouped values, concept values      | yes                    | yes (structural; across concepts `concept.mismatch`)                                                                                   | yes                                                 |
| `<` / `min` on concept values                                                                   | yes                    | only for `ordered concept`s represented by a quantity (`concept.no_order` otherwise)                                                   | yes                                                 |
...
Rules the syntax layer keeps out of itself: `ConceptId`, `DeclId`, grants,
...
_Both ends of the range must be comparable with angle._); `concept.no_order` for
an unordered concept; `formula.coalesce.not_optional` /
