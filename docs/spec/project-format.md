...
| `devices`  | `DeviceBinding`  | `id`, `name`, `kind` one of `pwm_channel`, `digital_output`, `digital_input`, `h_bridge_channel`, `i2c_sensor`, `quadrature_encoder`, `uart`, `output` (omitted for a device bound to no sink), `realization` (the output realization profile id, omitted when none; ADR-0036), `source` (the `DeclId` of the Source the device provides, omitted when none; exclusive with `output`; ADR-0038), `provider` (the Source provider profile id, omitted when none), `fixed_pins` (map from the device's requirement index to a board-relative pin name, omitted when empty) |
