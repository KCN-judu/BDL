...
the edit model and read by flattening (docs/architecture/behavior-systems.md
...
semantic undo across sessions are an open design issue (ISS-0009); the
`EditOp` type is already serializable so a log can be added without a new
vocabulary.
