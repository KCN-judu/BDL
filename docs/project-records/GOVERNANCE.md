...
|---|---|---|---|---|
| **Specification** | what BDL means now (language, files, protocol, kernel contract) | `docs/02-kernel-spec.md`, `docs/TEXTUAL_SYNTAX.md`, `docs/RUNTIME_SEMANTICS.md`, `docs/PROJECT_FORMAT.md`, `docs/PROTOCOL.md`, `docs/HARDWARE_MODEL.md`, `docs/STANDARD_CONCEPT_LIBRARY.md` | mutable current truth | — |
| **Architecture reference** | how the implementation is structured now | `docs/ARCHITECTURE.md` and the subsystem pages listed in [docs/README.md](../README.md) | mutable current truth | — |
| **Decision record** (ADR) | why an important choice was made, what else was considered | `docs/adr/NNNN-slug.md` | append-only once accepted | `ADR-NNNN` |
...
| **Implementation status** | what of the accepted design exists, per area | `docs/project-records/STATUS.md` | mutable | — |
...
| **Roadmap** | what is planned next, in order | `docs/ROADMAP.md` | mutable; never evidence that something exists | — |
| **Evidence** | which test, benchmark or theorem backs a claim | `docs/TESTING.md`, `docs/project-records/FORMAL_CORRESPONDENCE.md`, `docs/STUDIO_COMPILER_INTEGRATION.md` §3, `docs/user-guide/VERIFICATION.md`, the BDL_FV repository | mutable index of links | — |
...
| something got implemented, became partial, or was removed | `STATUS.md`, the roadmap row it completes | the ADR that designed it |
| users, project files, protocol clients or developers must react | a change fragment | prose inside the specification only |
| priorities moved | `ROADMAP.md` | `STATUS.md` |
| a formal result now (or no longer) backs a claim | `FORMAL_CORRESPONDENCE.md`, the affected spec/ADR's `fv` field | broadening the theorem in prose |

One change may touch several kinds — protocol 0.9 touched the specification
(`PROTOCOL.md`), status (daemon/textual rows), a change fragment
...
* **Accepted** ⇒ an ADR is written (the proposal's *Proposed design* is
  ...
  and links it). Accepted is not implemented: `STATUS.md` and the roadmap
  carry the work.
...
* The pre-2026-09-17 ledger `docs/DESIGN_ISSUES.md` (DI-1…DI-44) is
  archived as history; its open items were migrated as issues, its closed
  items keep their original text there.

### Implementation status

* `STATUS.md` has one row per area with a state — `implemented` ·
...
* Updated when work lands or is removed. When a roadmap item completes,
  its row moves out of the roadmap and into `STATUS.md` (capability) and,
...
without a YAML library. Everything else (specs, architecture, evidence)
has no frontmatter: the front door registers it, and the validator checks
that every `docs/*.md` is registered.
...
`FORMAL_CORRESPONDENCE.md` row: `formally proved` (the theorem proves
...
3. Sweep for stale claims in the authorities you did not touch:
   `grep -rn -E "planned|pending|not built|not yet|incomplete|TODO|deferred" docs/*.md docs/project-records`
...
are named in `docs/ARCHITECTURE.md`. If maintainers multiply, an
