...
old BDL documents did the opposite — `ROADMAP.md` carried ✅ marks and
ADR bodies carried phase checklists.

**BDL takes:** `STATUS.md` is the only place that says what exists, per
...
`DESIGN_ISSUES.md` had been used for — mixed with decided items and with
duplicate IDs.
...
**BDL takes:** every implemented claim in `STATUS.md` links a test; every
...
`engineering choice`); `FORMAL_CORRESPONDENCE.md` is the one table of what
...
| Accepted ≠ implemented: `STATUS.md`, not the ADR, says what exists | — | *Implemented* label | tracking issues — adopted as one matrix | Accepted vs Final — adopted as a rule, not as an ADR state |
