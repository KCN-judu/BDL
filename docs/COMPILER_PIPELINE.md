...
| 4 | surface elaboration | formula → `Expr` (`λ…mk B (… rep xᵢ …)`), units → scaled literals; calls → `app (declRef f) …`, `let` → `app (λ. …) v`, `match` → `ite`/`isSome`/`getD`, `Some`/`None`, `delay`/`sync` | grant, `rep`/`mk` | **integrated** for the textual expression forms (`bdl-elab::formula`; support matrix in `docs/TEXTUAL_SYNTAX.md` §11.1); `enum` constructors are syntax only (DI-19); `delay`/`sync` are the only temporal forms (the sugar phrases of `docs/TEXTUAL_SYNTAX.md` §12 are not parsed); curves, examples, components, contexts planned |
...
| 14 | reference evaluation / simulation | two-phase tick (read, write) over state cells `(DeclId, path)`; schedules; input traces; traces | `Ev` / `MEv` | **core rules complete, integrated** in bdld (`bdl-reactive::{eval,simulate}`); Studio Simulate page consumes it (docs/STUDIO_UI.md §10) |
