...
**Source templates** (`docs/spec/concept-library.md`): the _Sources_ category's
rows wear the Source silhouette (`MappingGlyph(source:)`), their names and
descriptions follow the locale (`TemplateText.displayNameIn`,
`AppLocalizations.libraryLocale`), and inserting one is the same request; the
answer's `created_mapping` is placed a node width (240 px) to the left of the
concept by the reducer, the concept opens for renaming as usual, and one Undo
removes both. The canvas menu's **Add Source ▸** lists the Source templates and
_New source…_, a sheet that is the New relationship form without its _Reads_ row
(`showNewSourceSheet`): the result is an ordinary `CreateMappingRequested` with
no inputs.
