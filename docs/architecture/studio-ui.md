...
- **Socket colour = concept identity.** In Blender a socket's colour is its data
  type; in BDL the type that matters is the nominal concept, so each concept
  gets a stable hue derived from its `ConceptId` (deterministic, never from the
  name; lightness chosen per hue so every identity clears 3:1 against the canvas
  in both appearances). A link is only accepted between sockets of the same
  concept — the canvas shows the nominal typing rule without a diagnostic: while
  a link is dragged every compatible socket gains a faint halo, the one under
  the pointer a strong halo, and an incompatible socket shows the forbidden
  cursor.
...
- **No concept node.** A concept is on the canvas only as the hue and the socket
  ...
  from the sidebar. _Add Block ▸ of C_ and its row dragged from the Project
  sidebar onto the canvas (`ConceptDrag`, `AddBlockRequested`) instantiate it at
  a point through the **block sheet** (`SourceSheetState.conceptId`: the Source
  sheet with the concept fixed, on screen at once, the name suggested by
  `blockNameFor` — `tilt`, `tilt2`), as often as the product has such values;
  two Sem blocks of one concept are ordinary and raise nothing (ADR-0044 §5).
...
- **Mapping block** (`NodeKind.definition`, `NodeRef.definition(id)` — the Sem
  ...
  **slot socket** per open position of the definition (`SocketRole.slot`, from
  ...
  (the layout service attaches it), and from then on each node moves alone. A
  base relationship a binding realises has no mapping block: the binding's edge
  into its filled realisation socket is its definition.
...
| drag a selected node                                                                                  | the selected movable set moves together, offsets kept, committed on release as one layout write (`NodesMoved`, no revision); a lone relationship dropped into or out of a region changes its membership; a mapping block grabbed moves alone (its own position) — its Sem block never follows it, nor it the Sem block                                                                                                |
...
| drop a Project-sidebar concept row                                                                    | the block sheet over that concept at the drop point (`AddBlockRequested`); its _Create_ is one `CreateSource` landing there                                                                                                                                                                                                                                                                                           |
...
identity: Studio sends the chosen `ConceptId`, or the new concept's description,
and infers nothing from names, value forms or units — the daemon ranks
(`bdl_library::rank_concepts`) and hides nothing.
...
| both                        | **Explain** (collapsed)        | `ConceptId` / `DeclId`, `Θ` / `Interface`, inferred type, core term, status enum, diagnostic codes and technical detail, revision, the last change's kind and invalidation categories                                                                                                                                                       | —                                                                                                                                                | level 3 only                                                                                                                 |
...
Words never shown at levels 1–2: `ConceptId`, `DeclId`, `Ty`, `q Dim`, `Grant`,
...
| **2 Inspector** | what does the selected object mean; what can I change; what will the change affect                       | designer vocabulary, per object: a concept — _Meaning, Value, Unit, Blocks, Rules_; a Sem block — _Meaning, Role, Relationship, Reads, Applies, Read by, Timing, Drives_; a rule — _Meaning, Role, Reads_ (the parameter editor), _Produces, Relationship, Depends on, Named in, Timing_; an output — _Output, Driver_; _affects, checked again_ on an edit; diagnostics in product language attached to the field they concern | `ConceptId`, `DeclId`, `Ty`, `Grant`, `Interface`, `realization`, `invalidation`, `Clocked`, `SingleDriver`, `DriveEnv`, `solver`, `protocol`, `revision`, enum names |
...
representation and nothing else; dashed is _not yet_ — a sink without a driver
or a domain — and nothing else; a red mark means _wrong now_ and never _not
yet_; a hollow socket at a _port_ means _unbound_ (its value form is known —
only a concept's hollow ring means _undecided_); region tint means _grouped_ and
never a domain or a category; the accent is selection, focus and the default
button. Adding an encoding adds a row here.
