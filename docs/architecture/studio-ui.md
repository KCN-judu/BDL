...
word in the right column; the description, and for a Source what it creates, on
hover). Recent items in the menu are a Studio preference, never project state.
...
**Source items**: inserting one is the same request; the answer carries
`created_concept` and `created_mapping`, the reducer places the relationship a
node width (240 px) to the left of the concept (the environment side), the
concept opens for renaming as usual, and one Undo removes both — the daemon
applied the fragment as one transaction. The canvas menu's **Add Source ▸**
lists the Source items and _New source…_, a sheet that is the New relationship
form without its _Reads_ row (`showNewSourceSheet`): the result is an ordinary
`CreateMappingRequested` with no inputs, not a library path.
