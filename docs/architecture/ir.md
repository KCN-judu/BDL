...
never sent to Studio. Grammar and precedence: `docs/spec/textual-syntax.md`.
...
inlined) and an evaluation order — `docs/architecture/executable-ir.md`. The Rust
backend prints it through a small owned AST — `docs/architecture/codegen-rust.md`.
...
Recorded so nobody mistakes them for the formal model (see `docs/archive/design-issues-ledger.md`):
