...
```
Editor integration outside the workspace: `editors/vscode` (a thin
client of `bdl-lsp`).
...
kernel gained no construct (FV Phase 8a, ADR-0021, `docs/evidence/behavior-systems-correspondence.md`).
...
`docs/architecture/codegen-rust.md`). The core is target-independent and knows no
...
See `docs/architecture/compiler-pipeline.md`. Diagnostics are first-class outputs: an
