...
|---|---|---|---|---|
| 0 | system elaboration (flattening) | `BehaviorSystem` → `ProjectSnapshot` + `OriginMap` + `system.*` diagnostics: instantiate, freshen private ids, substitute clock parameters, realise parameters, union, realise bindings as `Definition::Reference` | FV `flatten` (`docs/evidence/behavior-systems-correspondence.md`) | **integrated** (`bdl-system::flatten`); a flat project skips it — its snapshot *is* the input |
...
(`bdl_ide::draft_verdict`; docs/architecture/ide-service.md), so there is
...
`AnalyzeDefinitionDraft` (docs/spec/protocol.md) and Studio's definition editor
asks it as the designer types (docs/architecture/studio-compiler-integration.md).
...
differential tests (`docs/architecture/codegen-rust.md`); deployment feasibility is
...
`docs/guides/deployment-walkthrough.md`.
