...
How Studio exposes the compiler's semantic state and what it does not
expose yet. §3 is the evidence map (capability → service → protocol →
Studio state → UI → test) that `docs/project/status.md` cites for
the Studio row; the milestone's bug record lives in
`docs/changes/history/`.
...
cheap at this size (~2 ms at 400 mappings, docs/architecture/ide-service.md);
