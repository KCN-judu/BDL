...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | -------------------------------------- |
| 1   | The platform adapter's remaining half: a device that provides a Source's value on the board (ISS-0016 — the first adapter refuses a design with a Source), I²C and H-bridge sinks, the most the adapter supplies per list input; the core's numeric representation on device (ISS-0006). The first slice — output I/O, clock activation, the numeric policy at the boundary, the arena — exists (ADR-0037) | ISS-0016's device catalogue; an FV phase for stateful adapters and a device clock (ISS-0017) | ADR-0037, ADR-0004, ADR-0016, ADR-0027 |
| 2   | Build orchestration in `bdld`: `cargo check` / build of the generated crate (and its `--target` firmware) with structured progress and failures                                                                                                                                                                                                                                                            | —                                                                                            | —                                      |
| 3   | Flash through `probe-rs`                                                                                                                                                                                                                                                                                                                                                                                   | priority 2; runtime loading of `hardware/boards/` (the RP2040 board file exists)             | ADR-0015, ADR-0037                     |
...
| 5   | Third embedded family (ESP32-S3) — HAL independence is shown by the RP2040 (Embassy) and the Arduino Nano (avr-hal); the ESP32 adds a Wi-Fi-class target                                                                                                                                                                                                                                                   | priority 1                                                                                   | ADR-0037                               |
