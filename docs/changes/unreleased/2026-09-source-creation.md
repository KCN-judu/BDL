...
- **A Source is never created without a concrete concept.** The Source sheet
  ...
  or a **new** concept created in the same transaction (one revision, one Undo,
  both or nothing) — then the Source's name. The sheet previews exactly what
  will be committed; cancelling changes nothing; the `() -> ?` of an unmade
  choice exists only on the sheet.
- **The Standard Library's Source items are presets**, not identity-owning
  ...
  unit; a suggested Source name) and the daemon lists the design's concepts with
  that value form first. Two concepts of one value form stay two choices; an
  open value form is a legal choice; nothing is inferred from names, units or
  dimensions and nothing is hidden. Choosing an existing concept no longer
...
- **The presets are named for what they are** — _Temperature Input_, _Tilt
  ...
  Input_, _Analog Input_, _External Input_ (zh-Hans _温度输入_ …, ja _温度入力_
  …) — a Source is an environment boundary, not a chosen device. Their ids
  (`std.source.temperature`, …) did not change; their suggested names are
  `Temperature` / `temperatureInput` and so on.
...
- Library files: schema 2 unchanged. A `[[source]]` entry is read as a preset;
  ...
  **legacy path**, kept for third-party schema-2 libraries and older clients, no
  longer taken by Studio.
...
- Developers: `bdl_library::{SourcePreset, SourceCandidate, rank_concepts}`,
  ...
  `suggestedSourceName`), `PendingInsert.kSourceInsert`; `showNewSourceSheet` is
  gone; `UnitPreset` carries `typeName`.
...
`apps/studio/test/source_role_test.dart` (the sheet),
