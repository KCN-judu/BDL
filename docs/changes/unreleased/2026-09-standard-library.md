...
- **The Standard Concept Library is the Standard Library**: a catalogue of
  _items_, each a fragment of ordinary BDL objects, in two sections —
  _Concepts_ (36, one concept each) and _Sources_ (8, a concept and its
  `() -> concept` relationship). The Library tab shows the two sections with
  their groups; the Source rows wear the Source silhouette and say what they
  create. _External Value_ joins the Sources (a host, a network, a simulation —
  a Source is a boundary, not a sensor); _Button / Switch State_ is _Button
  State_; _Analog Input_ and _External Value_ leave the value form to the
  designer.
- **Instantiating an item is one transaction**: every object of the fragment
  or none. A Source item's concept and relationship are one revision, one
  history entry (one Undo removes both, a Redo recreates them with the same
  identities), and nothing is applied when any step is refused.
- **Item names, descriptions and search tags follow the language** — all 44
  ...
  localization; the library, the compiler service and the protocol carry
  English only, and what an item creates (`RoomTemp`, `TempSensor`) never
  changes with the locale. Search matches the localized words: `温度`,
  `传感器`, `センサー` find the Temperature Sensor.
...
- Protocol clients: protocol **0.17**, additive — `ListLibraryItems` /
  ...
  `display_names`, `descriptions` and `InstantiateConceptTemplateRequest.source_name`
  are deprecated and never set — a client that used them lists Sources through
  `ListLibraryItems` and localizes by item id. Refusals: `library.unknown_item`,
...
- Developers: `bdl_library::{LibraryItem, ItemCategory, Fragment,
  FragmentObject, ConceptSpec, MappingSpec, CreatedObject, PlannedStep, plan,
  free_name}`, `Library::{items, item, search_items}`, `LibrarySet::{items,
  item}`; `ConceptTemplate` is the projection `LibraryItem::as_concept_template`;
...
`crates/bdl-library/src/lib.rs` (unit tests), `crates/bdl-daemon/tests/stdio_e2e.rs`
(`library_items_over_stdio`), `crates/bdl-daemon/tests/text_e2e.rs`
