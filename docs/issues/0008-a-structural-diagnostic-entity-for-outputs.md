...
finding about a physical output (`output.missing_driver`,
`output.clock_unset`) names the sink in its message and the IDE lift
recovers the entity by matching the name.

## Why it matters

Structural anchoring is what every other entity has; name matching breaks
on a rename between analysis and display and cannot address a device.
...
* None; deferred until the next protocol change that touches
  `Diagnostic`.
