...
| Behaviour systems | — (bdld: `Session::apply_system`, `system_analysis`; the IDE host sees the derived flat design) | `InitSystemProject`, `GetSystem` → `SystemView` (components, ports, instances, bindings, exports, origins), `ApplySystemEdit`, `RunSystemAnalysis` (0.6); `ProjectProjection.kind`; `Definition.reference` | **none** | **none** — a system project opens as its derived flat design (`PROJECT_KIND_SYSTEM`); the inspector shows a bound port as a mapping with an empty formula; `ApplyEdit` on it is refused (`edit.derived_design`). The system canvas is the next milestone | Rust: `crates/bdl-system/tests/vertical_slice.rs` (15 cases), `bdl-daemon/tests/system_e2e.rs` |
