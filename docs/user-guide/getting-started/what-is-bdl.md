...
**concept**: a named thing the product senses, decides or shows. A concept has a
_value form_ — a quantity with a unit (an angle, a length, a temperature, or a
plain number), an on/off state, or a count.

Two concepts with the same value form are still different concepts. _Brightness_
and _Opacity_ may both be numbers between 0 and 1; BDL will not let you use one
where the other is expected, because they mean different things. This is
deliberate, and it is the first thing that makes a BDL design more than a
spreadsheet.

## 2. A relationship says how one value follows from others

`dimByTilt` reads _Tilt_ and produces _Brightness_. That is a **relationship**:
a named rule from some concepts to one concept. Its formula — `Tilt / 90 deg` —
is checked for units and dimensions: dividing an angle by an angle gives a plain
number, which is what a brightness is. Adding an angle to a time would be
refused, with the reason.

A relationship that reads nothing is a **value**: either something that arrives
from outside (a sensor reading, a switch) or something computed from other
...
one concept and is driven by exactly one relationship. A second relationship
trying to drive the same output is a conflict the tool names, not a race it
resolves. Which board the design will run on is a separate question:
**deployment** checks whether the outputs' devices can be placed on a chosen
board's pins, and never changes what the design means.
