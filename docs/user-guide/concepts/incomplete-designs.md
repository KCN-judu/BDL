...
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| **declared**    | the word _declared_ in the inspector; the status line's _not yet defined_ (a rule is not a node on the canvas)                                  | the relationship reads something and has no formula yet                                                                                       | only where its value is needed: an output it should drive stays undriven                 |
...
| **not applied** | the note _… is a rule nothing applies yet._ in the rule's inspector and on the Simulate page, with its fix (a rule is not a node on the canvas) | no value calls the rule, so nothing in the design has its result: the simulator shows no column for it and no output can be driven through it | no — the note on the Simulate page and in the inspector offers the value that applies it |

_Declared_, _open_ and _not applied_ are the intentional states. Where they are
drawn they are orange, hollow or dashed — never red. Red is reserved for _wrong
now_.
