...
| **Semantic tokens**  | `textDocument/semanticTokens/full`                                                           | the standard token types your editor already themes — `type` for a concept, `function` for a rule, `variable` for a value (with the modifier `source` for a Source, `output`, `device`, `instance`), `namespace` for a clock, `class` for a component, `property` for a port, `parameter`, `keyword`, `number`, `operator`, `comment`, `enumMember` — plus BDL's own `unit` (the word after a number) and `slot` (the `?`); `declaration` on a declared name, `defaultLibrary` on an equation of the library; the same classes Studio colours |
