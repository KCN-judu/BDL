...
| Concept representation | `Θ : SemanticId → Option Ty` (`q d` / `bool` / `nat`) | socket **shape**: ○ quantity, ◇ on-off, □ count | the kind word in the inspector | Kind: Quantity / On–off / Count / Decide later; Unit | "a measured value with dimension …" | spec (all circles today) |
...
| Formula validity | `HasType` under `Grant.of τ`, `Prim.ty` | node outline stays solid; validity is not a canvas mark | — | the definition editor's status line (one sentence, tone dot) and diagnostic rows under the field; spans underlined in the field; ladder word in *State* | "this formula gives a length, but `Brightness` is dimensionless" | now |
| Definition draft (uncommitted candidate) | none — Studio state (`DefinitionDraft`); the compiler judges it through `AnalyzeDefinitionDraft` | not on canvas (committed state only) | status line "N unsaved definitions" | *unsaved* in the section header; *Add definition* / *Save definition* / *Revert*; conflict notice *Reload* / *Keep mine* | "checked as it would be if committed; nothing is committed until you save" | now |
| Concept template (library) | none — `bdl-library` template id; a recipe for `CreateConcept` | not on canvas until inserted; the Library row's **grey** socket glyph (identity not allocated yet; filled = representation chosen, hollow = decide later); after insertion an ordinary concept node, name open for editing | the unit / kind word in the row's right column; the description on hover | Library tab rows; right-click *Add Concept ▸* | "a starting point: an ordinary concept with these defaults, yours to rename and rebind" | now |
| Dimensions | `q Dim` (8 exponents) | not on canvas | — | Unit pop-up: quantity-kind column, unit symbol column; first entry is the dedicated *no unit · dimensionless* choice | "angle · time⁻¹" | now |
