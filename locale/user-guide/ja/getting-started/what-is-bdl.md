...
式がわかる前に `dimByTilt` を作ることができます。そのとき、それは _宣言済み_ です。存在し、シグネチャを持ち、それに依存するすべてをその周りに設計できます。ツールはそれを未完の作業として示し、エラーとはしません——まだ式のないブロックを、別途指定するまで外部から提供される入力元として示すのと同じです。値の形をまだ選んでいないコンセプトも同様です。[未完成の設計](../../../../docs/user-guide/concepts/incomplete-designs.md)は、設計が取りうる状態と、そのどれも作業を止めない理由を説明しています。
