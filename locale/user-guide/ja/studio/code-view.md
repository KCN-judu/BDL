...
テキストは、各語がプロジェクトにとって _何であるか_ で色付けされます——綴りではありません。色はキャンバスのものです。**コンセプト**名は青みがかった灰色、**関係**はブロックの青、**入力元**は入力元ブロックの緑、**出力**や**デバイス**は出力ノードの暖色、**インスタンス**はインスタンスノードの青緑です。キーワード、演算子、単位は灰色、コメントはより薄い灰色です。名前は _宣言_ されている場所では使われている場所より太く、式の中だけに存在する名前——ルールのパラメータ、束縛子の変数——は斜体で、式に残された `?` は他と同じ _未決定_ のオレンジ色です。
