...
選択中のオブジェクトの**現在**と**実行全体**の値を、トレースと同じ書き方で、そのグリフとともに。出力なら、その駆動元も。コンセプトはそれを**運ぶ**ものとして示されます——_運搬元_、次にそれを生成する各値と入力元がその最新サンプルとともに。何もなければ _まだ何も Brightness を運んでいません：それを生成する値も入力元もありません。_ と、それを生成する各規則について _dimByTilt は規則です。その数式がそれを適用する値が Brightness を運びます。_ 規則には示す値がありません：_規則：自身の値を持ちません。その数式がそれを適用する値が、シミュレータがサンプルするものです。_、次に _適用先_ の下に、数式がそれを適用する値へのリンク——または _まだどの値もそれを適用していません。_ と準備状態の領域が提示するのと同じ修正。その下の**説明**は識別番号、実行のリビジョン、失敗ならコードと技術的なテキストを保持します。
