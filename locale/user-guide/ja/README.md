...
ツールの背後にある言語は **BDL** と呼ばれます。ツールを使うのに言語を知る必要はありません。キャンバス、インスペクター、数式フィールドが言語_そのもの_です。エディタを好む人向けに BDL のテキスト形式もあります。[テキスト形式の BDL](../../../docs/user-guide/textual/overview.md) を参照してください。
