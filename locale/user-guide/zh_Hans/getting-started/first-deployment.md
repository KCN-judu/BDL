...
结论变为**可部署到 Arduino Nano。**，并且 _arduino_nano 上的布置_ 表格显示这盏灯需要的那一行：`pwmLight`、它的 _PWM_ 需求，以及分配给它的引脚，例如 `D3`。
...
清空该栏：又可部署了。现在把**目标板**切换到 **Big board (mock)**：那里也可部署，只是引脚不同。再切回来。
