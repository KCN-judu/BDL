...
选中对象**当前**和**整个运行过程中**的值，写法与轨迹相同，带有它的字形；对于输出，则是它的驱动者。概念通过它的**块**来显示——_块_，然后是该概念的每个块及其最新采样，每个每 tick 一个值；没有块时显示 _尚无 Brightness 的块：从画布菜单添加一个。_ 规则没有值可显示：_规则：它没有自己的值。公式应用它的值才是仿真器采样的对象。_，然后在 _应用于_ 下列出公式应用它的值的链接——或者 _尚无值应用它。_，附带就绪区提供的同一修复。下方的**解释**保存标识号、运行的修订号，以及失败时的代码和技术文本。
